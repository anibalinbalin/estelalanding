...
const ValueCard = ({ title, description, currentTheme }: { title: string; description: string; currentTheme: string }) => {
  return (
    <div className={`col-span-12 min-[600px]:col-start-[2] min-[1000px]:col-span-4 min-[600px]:col-span-10 rounded-[var(--border-radius-lg)] min-[600px]:rounded-[var(--border-radius-xl)] min-[1000px]:rounded-[var(--border-radius-lg)] border shadow-[var(--elevation-1)] ${
      currentTheme === 'light' 
        ? 'bg-[#ffffff] border-[#e4e5e4]' 
        : 'bg-[var(--surface-raise-p3)] border-[var(--stroke-secondary-p3)]'
    }`}>
